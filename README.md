...
**Using Gemini free tier instead of OpenAI (optional)**: switch to Google's free-tier model by setting `MODEL=gemini/gemini-2.5-flash-lite` in your `.env`. You only need one key (`GEMINI_API_KEY`) because the same key covers both the chat model and embeddings — no credit card required. Get a key at [ai.google.dev](https://ai.google.dev). The `MODEL` env var works with any provider `litellm` supports, so you can also swap in other models (e.g. `deepseek/deepseek-chat` with `DEEPSEEK_API_KEY`, or `cohere/command-r-plus` with `COHERE_API_KEY` — Cohere's free trial key is capped at 1,000 calls/month and chat only, embeddings still need `GEMINI_API_KEY`) without touching the code.

**Hit a Gemini "RESOURCE_EXHAUSTED" / quota error?** The free tier's daily request quota is tracked **per model**, not per key — switching `MODEL` to a different free Gemini model (e.g. `gemini/gemini-2.5-flash`, `gemini/gemini-2.5-flash-lite`, `gemini/gemini-3.5-flash`) gets you a fresh quota immediately, no new key needed. Avoid `gemini-2.0-flash`/`gemini-2.0-flash-lite` — both are deprecated and no longer on the free tier. `gemini-2.5-pro` has no free tier at all (paid only).
