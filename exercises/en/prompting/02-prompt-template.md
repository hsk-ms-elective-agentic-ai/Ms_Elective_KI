...
2. Run the cells in each notebook (Setup cell first, then the exercise cell) — 2b runs two worked examples directly (teacher vs. engineer persona); 2a, 2c, 2d, and 2e have `TODO` fields to fill in first. Each notebook's own "Your task" section has the specific comparison question for that technique.
