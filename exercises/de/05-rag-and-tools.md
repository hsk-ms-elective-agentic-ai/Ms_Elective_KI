...
4. Aktualisiert die Task-Beschreibung dieses Agenten in `tasks.yaml` so, dass sie eine Frage enthält, die nur aus eurem Wissensdokument beantwortet werden kann. Führt es aus — entweder öffnet [step_05_rag_and_tools.ipynb](../en/step_05_rag_and_tools.ipynb) (auf Englisch) und führt die Zelle aus, oder im Terminal:
   ```bash
